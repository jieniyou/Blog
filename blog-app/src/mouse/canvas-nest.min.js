! function() {
  ...
  function b() {
    ...
    t.forEach(function(i) {
      i.x += i.xa, i.y += i.ya, i.xa *= i.x > r || i.x < 0 ? -1 : 1, i.ya *= i.y > n || i.y < 0 ? -1 : 1, e
        .fillRect(i.x - 0.5, i.y - 0.5, 1, 1)
      for (v = 0; v < w.length; v++) {
        x = w[v]
        if (i !== x && null !== x.x && null !== x.y) {
          B = i.x - x.x, z = i.y - x.y, y = B * B + z * z
          y < x.max && (x === f && y >= x.max / 2 && (i.x -= 0.03 * B, i.y -= 0.03 * z), A = (x.max - y) / x.max, e
            .beginPath(), e.lineWidth = A / 2, e.strokeStyle = 'rgba(' + s.c + ',' + (A + 0.2) + ')', e.moveTo(i
              .x, i.y), e.lineTo(x.x, x.y), e.stroke())
        }
      }
